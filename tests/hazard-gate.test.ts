...
describe("hazard gate in runScript", () => {
  ...
  // --- bash ---
  it("blocks bash script with rm -rf", async () => {
  ...
  it("allows bash with --force even if dangerous", async () => {
    createScript(NAME, "bash", "test");
    writeScript("#!/bin/bash\nrm -rf /tmp/foo\n");
    const res = await runScript(NAME, { force: true });
    // Gate bypassed — execution might fail on Windows, but NOT blocked by hazard
    expect(res.output).not.toContain("hazard gate");
  });

  it("blocks bash with sudo", async () => {
    createScript(NAME, "bash", "test");
    writeScript("#!/bin/bash\nsudo rm /tmp/x\n");
    const res = await runScript(NAME);
    expect(res.ok).toBe(false);
  ...
  it("blocks bash with curl-pipe-shell", async () => {
    createScript(NAME, "bash", "test");
  ...
  // --- safe scripts pass ---
  it("passes safe bash script", async () => {
    createScript(NAME, "bash", "test");
    writeScript('#!/bin/bash\necho \'{"status":"success"}\'\n');
    const res = await runScript(NAME);
    expect(res.output).not.toContain("hazard gate");
  ...
  it("passes safe node script", async () => {
  ...
  it("records lastForceRun when run with --force", async () => {

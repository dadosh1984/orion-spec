...
export interface RouterDecision {
  action: "USE_EXISTING_SKILL" | "CREATE_NEW_SKILL" | "DIRECT_AI" | "ASK_USER" | "REJECT";
  skillName?: string;
  confidence: number;
  reason: string;
}

export function findExistingSkill(prompt: string): { name: string; score: number } | null {
  ...
  try {
    for (const d of readdirSync(dir, { withFileTypes: true })) {
      ...
      try {
        const m = JSON.parse(readFileSync(mf, "utf8")) as { name: string; description: string };
        let score = 0;
        const desc = (m.description || "").toLowerCase();
        for (const w of words) { if (d.name.includes(w)) score += 3; if (desc.includes(w)) score += 1; }
        if (score > 0 && (!best || score > best.score)) best = { name: d.name, score };
      } catch { /* skip */ }
    }
  } catch { /* skip */ }
  return best;
...
export function routeRequest(prompt: string): RouterDecision {
  ...
  if (existing && existing.score >= 5) {
    return { action: "USE_EXISTING_SKILL", skillName: existing.name, confidence: Math.min(0.9, existing.score / 10), reason: `Found matching skill "${existing.name}" (score: ${existing.score}).` };
  }
  const cat = classifyTask(prompt);
  if (cat.category <= 3) return { action: "CREATE_NEW_SKILL", confidence: 0.8, reason: `${cat.label}. ${cat.reason}` };
  if (cat.category === 4) return { action: "CREATE_NEW_SKILL", confidence: 0.6, reason: `${cat.label}. ${cat.reason}` };
...
export function verifyRun(stdout: string, postconditions?: Postcondition[]): VerifyResult {
  ...
  for (const pc of postconditions ?? []) {
    if (pc.type === "json_field" && parsed) {
      ...
      checks.push({ name: `json_field ${pc.field} == ${JSON.stringify(pc.equals)}`, passed, detail: passed ? "ok" : `expected ${JSON.stringify(pc.equals)}, got ${JSON.stringify(val)}` });
      ...
      checks.push({ name: `metric ${pc.field} >= ${pc.min}`, passed, detail: passed ? "ok" : `${pc.field}=${val}, min=${pc.min}` });
    } else if (pc.type === "file_exists") {
      const p = pc.path ?? "";
      const passed = existsSync(p);
      checks.push({ name: `file_exists ${p}`, passed, detail: passed ? "exists" : "not found" });
    }
  }
...
function readWatchers(): WatcherEntry[] {
  try { if (!existsSync(WATCHER_REGISTRY)) return []; return JSON.parse(readFileSync(WATCHER_REGISTRY, "utf8")) as WatcherEntry[]; }
  catch { return []; }
}
...
export function addFileWatcher(name: string, watchDir: string, pattern: string, skillName: string): WatcherEntry {
  const entries = readWatchers().filter((e) => e.name !== name);
  const entry: WatcherEntry = { name, watchDir, pattern, skillName, createdAt: new Date().toISOString() };
  entries.push(entry);

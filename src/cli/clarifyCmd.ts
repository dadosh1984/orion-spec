...
export async function clarifyCommand(args: string[]): Promise<number> {
  ...
  if (!changeId) {
    console.error(
      "orion: clarify requires a change id, e.g. orion clarify my-change",
    );
    return 1;
  }
...
export async function answerCommand(
  ...
  if (!changeId || !jsonPath) {
    console.error(
      "orion: answer requires: orion answer <change-id> <answers.json>",
    );
    return 1;
  }
  if (!existsSync(`changes/${changeId}`)) {
  ...
  if (Array.isArray(parsed)) {
    ...
    console.error(
      "orion: invalid answers format — expected array or { answers: [] }",
    );
    return 1;
  }
  ...
  for (const a of answers) {
    if (!a.questionId || typeof a.text !== "string") {
      console.error(
        `orion: invalid answer entry — must have "questionId" and "text"`,
      );
      return 1;
    }
  ...
  if (remaining) {
    console.log(
      `${EMOJI.blocker} Blockers remaining — use orion clarify ${changeId} to review`,
    );
  } else {
    console.log(
      `${EMOJI.done} All blockers resolved! Ready for orion out ${changeId}`,
    );
  }
  return 0;
}
...
export async function refineCommand(
  ...
  if (!changeId) {
    console.error(
      "orion: refine requires a change id, e.g. orion refine my-change",
    );
    return 1;
  }
  if (!existsSync(`changes/${changeId}`)) {
  ...
  try {
    ...
    console.log(
      `${EMOJI.done} Context refined for "${changeId}" (${answered} answer(s) merged).`,
    );

    if (blockersMsg) {
      console.error(`${EMOJI.blocker} ${blockersMsg}`);
      console.error(
        "  Resolve blockers with orion answer, then retry refine --auto.",
      );
      return 1;
    }
    ...
    console.error(
      `orion: refine failed: ${err instanceof Error ? err.message : String(err)}`,
    );
    return 1;
  }
}

...
export function historyCmd(args: string[]): { ok: boolean; text: string } {
  ...
  if (rows.length === 0) {
    return {
      ok: true,
      text: `${statusMark("info")} No history yet. Run \`orion shell\` to start recording.`,
    };
  }
  ...
  return {
    ok: true,
    text: [
      `${statusMark("info")} History (${rows.length} entries, last ${display.length}):`,
      ...display.map(
        (l, i) =>
          `  ${String(rows.length - display.length + i + 1).padStart(3)}  ${l}`,
      ),
    ].join("\n"),
  };
}

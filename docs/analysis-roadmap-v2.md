...
### Фаза 2 — Интерактивность и термин-вывод (0.31.0)
1. `src/utils/term.ts` (цвета/иконки/бары) + единый `render-helpers`.
2. Единый язык сессии (`session.lang`, A7) для всех команд.
3. Summary после каждой команды + «что дальше» (B11-161, 170).
4. `--no-color` / NO_COLOR + консистентные эмодзи (B7-102, 103).
5. Golden-тесты на вывод (B8-117).

### Фаза 3 — Производительность (0.32.0)
1. Ленивые import() MCP-тулов (B6-81) — замер startup.
2. Мемоизация readProfile/loadDenyList/scanChanges (B6-82..84).
3. Дебаунс авто-refresh дашборда (B6-85).
4. Параллелизм shield-проверок (B6-86).
5. benchmarks в cache-bench (B4-55).

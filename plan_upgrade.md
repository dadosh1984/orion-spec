...
>
> **A1 — БЕЗОПАСНЫЙ ПРОМОУШЕН (2026-08-14, закрыт):**
> - `orion run match --approve "<sig>"` — scaffold изменения `changes/<slug>/` из
>   miss-log: replay-задачи (историч. вход→ожидаемый выход) + требование ≥3 повторов
>   + ручной `entry.js` → `forge --save-as`. Авто-register невозможен (безопасно,
>   никакого тихого скрипта).

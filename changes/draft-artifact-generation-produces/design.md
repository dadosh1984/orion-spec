# Дизайн — draft-artifact-generation-produces
...
- `changes/draft-artifact-generation-produces/snippets/*` — подсказки реализации по задачам

## Допущения
- Reproduce the failure: write/run a test that fails on the current code (RED)
